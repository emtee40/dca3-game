...
Geometry::create(int32 numVerts, int32 numTris, uint32 flags)
{
	...
	if(!(geo->flags & NATIVE)){
		int32 sz = 0;
		if (geo->flags & HAS_TRIANGLES)
			sz += geo->numTriangles*sizeof(Triangle);
		if(geo->flags & PRELIT)
...
Geometry::destroy(void)
{
	...
	if(this->refCount <= 0){
		...
		// Also frees triangles, colors and tex coords
		rwFree(this->data);
...
Geometry::streamRead(Stream *stream)
{
	...
	Geometry *geo = Geometry::create(buf.numVertices,
	                                 buf.numTriangles, buf.flags | HAS_TRIANGLES);
	#else
	Geometry *geo = Geometry::create(buf.numVertices,
	...
	// We only support loading NATIVE assets on dreamcast build
	assert(geo->flags & NATIVE);
	#endif
	...
	if(!(geo->flags & NATIVE)){
		...
		for(int32 i = 0; i < geo->numTexCoordSets; i++)
		{
			for (int32 j = 0; j < geo->numVertices; j++)
			{
				...
				stream->read32(&u, 4);
				stream->read32(&v, 4);
				geo->texCoords[i][j].u  = u;
				geo->texCoords[i][j].v  = v;
			}
		}
		for(int32 i = 0; i < geo->numTriangles; i++){
			uint32 tribuf[2];
...
Geometry::streamWrite(Stream *stream)
{
	...
	// We only support writting NATIVE assets on dreamcast build
	assert(this->flags & NATIVE);

	if(!(this->flags & NATIVE)){
		if(this->flags & PRELIT)
			stream->write8(this->colors, 4*this->numVertices);
		for(int32 i = 0; i < this->numTexCoordSets; i++)
			// Convert UVs back to f32 for writes
			for (int j = 0; j < this->numVertices; j++) {
				float u = this->texCoords[i]->u;
				stream->write32(&u, 4);	
				float v = this->texCoords[i]->v;
				stream->write32(&v, 4);	
			}
		for(int32 i = 0; i < this->numTriangles; i++){
			uint32 tribuf[2];
			// is this really not required? if so, remove?
