...
Clump::streamWrite(Stream *stream)
{
	...
	if(rw::version >= 0x30400){
		size = 12+4;
		{
			// Validate that all atomics have different geometries
			std::set<rw::Geometry*> geoms;
			FORLIST(lnk, this->atomics) {
				assert(!geoms.count(Atomic::fromClump(lnk)->geometry));
				geoms.insert(Atomic::fromClump(lnk)->geometry);
