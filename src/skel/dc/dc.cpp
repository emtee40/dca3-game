...
// //TODO: these are somehow missing
...
#if !defined(DC_SIM) && defined(WITH_IDE)
static kos_blockdev_t ide_rv;

static void ide_fat_init(void)
{
	uint8_t type;
	int err;
	...
	err = g1_ata_blockdev_for_partition(0, 1, &ide_rv, &type);
	if (err)
		return;
	...
	err = fs_fat_mount("/ide", &ide_rv, FS_FAT_MOUNT_READONLY);
	if (err)
		return;
...
#if !defined(DC_SIM)
#if defined(WITH_PROF)
static bool profiling = false;
#endif
#if defined(WITH_SD)
static kos_blockdev_t sd_rv;

static void sd_fat_init(void)
{
	uint8_t type;
	int err;
	...
	err = sd_blockdev_for_partition(0, &sd_rv, &type);
	if (err)
		return;

	printf("Found SD partition 0\n");

	err = fs_fat_init();
	if (err)
		return;

	err = fs_fat_mount("/sd", &sd_rv, FS_FAT_MOUNT_READWRITE);
	if (err)
		return;

	printf("Mounted SD partition 0 to /sd\n");
}
#endif
#endif
...
#else
	static char szUserFiles[256];
	strcpy(szUserFiles, "/vmu/" VMU_DEFAULT_PATH);
	// _psCreateFolder(szUserFiles);
	return szUserFiles;
#endif
}
...
psCameraShowRaster(RwCamera *camera)
{
	if (CMenuManager::m_PrefsVsync)
		RwCameraShowRaster(camera, NULL /*PSGLOBAL(window)*/, rwRASTERFLIPWAITVSYNC);
	else
		RwCameraShowRaster(camera, NULL /*PSGLOBAL(window)*/, rwRASTERFLIPDONTWAIT);
...
psInitialize(void)
{
	...
	// sysinfo(&systemInfo);
	_dwMemAvailPhys = 10 * 1024 * 1024 ; //systemInfo.freeram;
	// debug("Physical memory size %u\n", systemInfo.totalram);
	// debug("Available physical memory %u\n", systemInfo.freeram);
#endif
...
#define SDL_GAMEPAD_DB_PATH "gamecontrollerdb.txt"
	FILE *f = fcaseopen(SDL_GAMEPAD_DB_PATH, "rb");
	if (f) {
		fseek(f, 0, SEEK_END);
...
main(int argc, char *argv[])
{
	...
	/* Disable printf() buffering, because it's slow with latest Newlib/toolchain combo. */
	setvbuf(stdout, NULL, _IONBF, 0);
#if defined(MASTER) || defined(NDEBUG)
    dbglog_set_level(DBG_CRITICAL);
#endif

    printf("sbrk: %p\n", sbrk(0));
    printf("Loading game assets from %s\n", STR(DC_CHDIR));
    chdir(STR(DC_CHDIR));
	...
	sd_fat_init();
	#endif
	cont_btn_callback(0, CONT_A | CONT_B | CONT_X, [](uint8_t, uint32_t btns) {
...
#if !defined(_WIN32) && !defined(__SWITCH__) && !defined(RW_DC)
...
#if defined(WITH_PROF) && !defined(DC_SIM)
	if(profiling) {
		profiling = false;
		profiler_clean_up();
	}
